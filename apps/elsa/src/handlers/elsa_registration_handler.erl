...
validate(Req, State, invalid_content_type) ->
  ...
  {ok, elsa_handler:error(400, <<"Content type must be application/json">>, Req), State};
...
validate(Req, State, false) ->
  ...
  {ok, elsa_handler:error(400, <<"Missing Body">>, Req), State};
...
validate(Req, State, invalid_json) ->
  ...
  {ok, elsa_handler:error(422, <<"Not valid JSON">>, Req), State};
...
validate(Req, State, invalid_schema) ->
  ...
  {ok, elsa_handler:error(422, <<"Schema does not match">>, Req), State}.
...
update(Req, State) ->
  ...
  case cowboy_req:method(Request) of
    {<<"POST">>, _} ->
        ...
        {ok, elsa_handler:message(200, <<"Successfully registered">>, Request), State};
    {<<"DELETE">>, _} ->
      elsa_registry:unregister(Body),
      {ok, elsa_handler:message(200, <<"Successfully unregistered">>, Request), State}
  end.
